...
function addOptions(b,c,d){
    if(b.length>0) {
	var dr = document.getElementById("addopt");
	for(var j = 0; j < b.length; j++) {
	    if(j>0) {
		addOpt(dr,b[j],c[j],d[j]);
	    }
	}
    }
}
...
function addOpt(btn,value,oid,cnt){
    btns = document.getElementsByName('drop');
	...
	for (var i = 0; i < thisChilds.length; i++){
		...
		str = '<input type="hidden" id="cnt" name="cnt" value="'+cnt+'">\n';
		str = str + '<input type="hidden" id="oid" name="oid" value="'+oid+'">\n';
		str = str + '<input type="text"   id="opt" name="opt" value="'+value+'" maxlength="255">\n';
		str = str + '<input type="button" name="drop" value=" &minus; " onclick="dropOpt(this);">\n';
		str = str + '<input type="button" id="addopt" value=" + " onclick="addOpt(this,\'\',0,0);">';
